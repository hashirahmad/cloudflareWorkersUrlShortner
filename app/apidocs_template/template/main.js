...
require([
	...
	var docs_location = location.origin;
	var index_to = location.pathname.indexOf("docs/") - 1;
    ...
    apiGroups.forEach(function(groupEntry) {
        ...
        api.forEach(function(entry) {
            if(groupEntry === entry.group) {
				...
				try
				{
					 fields.article.sampleRequest[0].url = fields.article.sampleRequest[0].url.replace( "DYNAMIC", docs_location);
				}
				catch(e)
				{
                }
